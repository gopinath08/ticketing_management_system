...
include("../../user.php");
$id=$_REQUEST['id'];
$stmt = $con->prepare("SELECT tickets.id as tickets_id,tickets.client_id,tickets.hod_id,tickets.subject,tickets.description,tickets.tickets_status,tickets.created_on,masters_project.Project_name,tickets.tickets_status,task_assign.due_date,task_assign.users_id,task_assign.hours FROM `task_assign` INNER JOIN tickets ON task_assign.Task_id=tickets.id INNER JOIN masters_project ON tickets.project_id=masters_project.id where Task_id='$id'"); 
$stmt->execute(); 
$row = $stmt->fetch();
?>
 <div class="card card-info">
              ...
              <!-- /.card-header -->
              <!-- form start -->
<form class="form-horizontal" method="POST" action="Tickets/users/users_submit.php">
